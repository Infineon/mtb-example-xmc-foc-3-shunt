...
#if (MOTOR0_PMSM_FOC_BOARD == EVAL_6EDL7151_FOC_3SH)
uint8_t GUIwrReg_6EDL7151_addr = { 0x55 };
uint8_t WriteReg_6EDL7151_addr = { 0x55 };
uint16_t GUIwrReg_6EDL7151_data;
uint16_t WriteReg_6EDL7151_data;
uint8_t GuiMonitor_6EDL7151_addr;
uint16_t GuiMonitor_6EDL7151_value;

#if (GUI_6EDL7151_INTEGRATION != DISABLED)
...
const XMC_GPIO_CONFIG_t GPIO_EN_OFFSTATE_Config = { .mode = XMC_GPIO_MODE_OUTPUT_PUSH_PULL, .output_level = XMC_GPIO_OUTPUT_LEVEL_HIGH, };

EDL_IO_CONTROL_t EdlIo;
edl7151_register_t Edl7151Reg;

/**
 * @brief Default 6EDL7151 register setting for testing purpose if macro PROGRAM_DEFAULT_PARAM set to 1U
 *
 * @param None
 *
 * @retval None
...
void EDL7151_MOTOR_PARAM_set_default(void)
{
  ...
  Edl7151Reg.VDS_SENSE_CFG = VDS_TH_2000mV << VDS_SENSE_CFG_HS_TH_Pos |
                         VDS_TH_2000mV << VDS_SENSE_CFG_LS_TH_Pos |
               VDS_5_0us << VDS_SENSE_CFG_BLANK_Pos |
               VDS_5_0us << VDS_SENSE_CFG_FILTER_Pos|
               SENSOR_ENABLE << VDS_SENSE_CFG_SENSE_EN_Pos;
  Edl7151Reg.FAULTS_CLR = 0 << FAULTS_CLR_CLR_FLTS_Pos | 
                          0 << FAULTS_CLR_CLR_LATCH_Pos;
  Edl7151Reg.SUPPLY_CFG = PVCC_12V << SUPPLY_CFG_PVCC_SETPT_Pos |
  ...
  Edl7151Reg.ADC_CFG =    0 << ADC_CFG_ADC_OD_REQ_Pos |
  ...
  Edl7151Reg.IDRIVE_CFG = IDRIVE_200mA << IDRIVE_CFG_IHS_SRC_Pos |
                          IDRIVE_200mA << IDRIVE_CFG_IHS_SINK_Pos |
                          IDRIVE_200mA << IDRIVE_CFG_ILS_SRC_Pos |
                          IDRIVE_200mA << IDRIVE_CFG_ILS_SINK_Pos;
  Edl7151Reg.IDRIVE_PRE_CFG =   IDRIVE_PRE_200mA << IDRIVE_PRE_CFG_I_PRE_SRC_Pos |
                                IDRIVE_PRE_200mA << IDRIVE_PRE_CFG_I_PRE_SINK_Pos |
                                PRECHAR_MODE_EN  << IDRIVE_PRE_CFG_I_PRE_SRC_DIS_Pos |
                                PRECHAR_MODE_EN  << IDRIVE_PRE_CFG_I_PRE_SNK_DIS_Pos |
                                GATE_CLAMP_EN    << IDRIVE_PRE_CFG_I_CLAMP_DIS_Pos;
  Edl7151Reg.TDRIVE_SRC_CFG =   TDRIVE1_190ns << TDRIVE_SRC_CFG_TDRIVE1_Pos |
                                TDRIVE2_150ns << TDRIVE_SRC_CFG_TDRIVE2_Pos;
  Edl7151Reg.TDRIVE_SINK_CFG =  TDRIVE3_190ns << TDRIVE_SINK_CFG_TDRIVE3_Pos |
                                TDRIVE4_150ns << TDRIVE_SINK_CFG_TDRIVE4_Pos;
  Edl7151Reg.DT_CFG =     DT_840ns << DT_CFG_DT_RISE_Pos |
                          DT_840ns << DT_CFG_DT_FALL_Pos;
  Edl7151Reg.CP_CFG =     CP_CLK_781_25kHz << CP_CFG_CP_CLK_CFG_Pos |
                          CP_CLK_SS_EN << CP_CFG_CP_CLK_SS_DIS_Pos;
  Edl7151Reg.CSAMP_CFG =  CS_GAIN_12V << CSAMP_CFG_CS_GAIN_Pos |
  ...
  Edl7151Reg.CSAMP_CFG2 = OCP_POS_THR_70mV << CSAMP_CFG2_CS_OCP_PTHR_Pos |
  ...
  Edl7151Reg.OTP_PROG =   0 << OTP_PROG_OTP_PROG_Pos | 
                          0xF << OTP_PROG_USER_ID_Pos; /* write a value different to the value in OTP, this helps to detect unexpected IC reset */

#endif /* (VALID_FLASH_DATA == 0x1) */
  Edl7151Reg.valid = VALID_FLASH_DATA;
}

/**
 * @brief If macro PROGRAM_DEFAULT_PARAM is 1U, it copy default 6EDL7151 register values to FLASH,
 * else it read the chip version from Flash address, if match, it will upload the 6EDL7151 register value from FLASH to RAM,
...
void EDL7151_FLASH_parameter_load(void)
{
  ...
  XMC_FLASH_ProgramVerifyPage(Edl7151ParameterBlock_Addr, (uint32_t*) &Edl7151Reg);
  SystemVar.Edl7151Configured = 1;
#else
  Edl7151Reg.valid = ((edl7151_register_t*)Edl7151ParameterBlock_Addr)->valid;
  if (Edl7151Reg.valid != VALID_FLASH_DATA)
  {
    ...
    SystemVar.Edl7151Configured = 0;
  }
  else
  {
    /* Found parameter block with matching version, copy parameter block data into Edl7151Reg structure */
    edl7151_register_t *parameter_block = (edl7151_register_t*)Edl7151ParameterBlock_Addr;
    for (int32_t i = 0; i < CFG_ADDR_MAX; i++)
    {
      Edl7151Reg.table[i] = parameter_block->table[i];
    }
    SystemVar.Edl7151Configured = 1;
  }
...
/**
 * @brief To read all the 6EDL7151 registers when startup
 *
 * @param None
 *
 * @retval None
...
void SPI_read_6EDL7151_registers(void)
{
  ...
  {
    Edl7151Reg.table[addr] = read_word_16b(addr);
  }
...
/**
 * @brief To write the 6EDL7151 configuration registers to default values
 *
 * @param None
 *
 * @retval None
...
void SPI_write_6EDL7151_registers(void)
{
    ...
    for (addr = CFG_ADDR_MIN; addr < CFG_ADDR_MAX; addr++)
    {
        if (addr != CFG_ADDR_MIN + 1)                   /* skip writing FAULT_CLR register at address 0x10 */
        {
          write_word_16b(addr, Edl7151Reg.table[addr]);
        }
    }
...
void EDL7151_Config_init(void)
{
  ...
  /* To write the 6EDL7151 configuration registers to default values */
  SPI_write_6EDL7151_registers();
  /* read all the 6EDL7151 registers to Edl7151Reg table */
  SPI_read_6EDL7151_registers();
}
...
{
  /* Initializes P4_9 to bias SHx node */
  uint8_t addr = ADDR_SENSOR_CFG;
  uint16_t data	;
  data = read_word_16b(addr);                                      /* read back data at SENSOR_CFG  */
  if (EdlIo.en_off_state_diagnostic == 1)
  {
    data |= VDS_OFF_DIAG_EN << SENSOR_CFG_VDS_OFF_DIAG_EN_Pos;   /* enable VDS_OFF_DIAG */
  ...
  {
    data &= VDS_OFF_DIAG_DIS << SENSOR_CFG_VDS_OFF_DIAG_EN_Pos;  /* disable VDS_OFF_DIAG */
    XMC_GPIO_SetOutputLow(GPIO_EN_DRV);                          /* set EN_DRV pin low */
    EdlIo.en_drv_level = 0;
  }
  write_word_16b(addr, data);
  data = read_word_16b(addr);                                      /* read back data at SENSOR_CFG  */
  if ((data & SENSOR_CFG_VDS_OFF_DIAG_EN_Msk) >> SENSOR_CFG_VDS_OFF_DIAG_EN_Pos == 1) /* OFF State diagnostic mode is enable */
  {
    XMC_GPIO_SetOutputHigh(GPIO_EN_DRV);                         /* set EN_DRV pin High */
    EdlIo.en_drv_level = 1;
    PMSM_FOC_Clock_Init();                                       /* Reset configuration, clock configuration */
    XMC_SCU_EnablePrefetchUnit();                                /* Enable prefetch unit */
    XMC_GPIO_SetOutputHigh(PHASE_U_LS_PIN);                      /* Init GPIO PIN to toggle INLx, INHx */
    XMC_GPIO_SetOutputHigh(PHASE_V_LS_PIN);                      /* Init GPIO PIN to toggle INLx, INHx */
    XMC_GPIO_SetOutputHigh(PHASE_W_LS_PIN);                      /* Init GPIO PIN to toggle INLx, INHx */
    for (int x = 1000; x >0; x--) {};                            /* create a delay loop*/
    XMC_GPIO_SetOutputLow(PHASE_U_LS_PIN);
    XMC_GPIO_SetOutputLow(PHASE_V_LS_PIN);
    XMC_GPIO_SetOutputLow(PHASE_W_LS_PIN);
    for (int x = 100; x >0; x--) {};                             /* create a delay loop*/
    ...
    addr = ADDR_FUNCT_ST;
    data = read_word_16b(addr);                                  /* read back data at FUNCTION STATUS register  */
    data &= 0x3F00;
    data = data >>8;
    if (data != 0x3f)                                            /* OFF State diagnostic detect FET error */
    {
      SystemVar.OffState_fault = data;
      Edl7151Reg.table[0x3] |= data << 8;                      /* Update the VDS status value to FUNCT_ST register*/
    }
    else                                                         /* OFF State diagnostic detect no FET error */
    {}
  }
  else                                                             /* OFF State diagnostic mode is disable */
  {
    if (SystemVar.OffState_fault == 0x3f)                        /* No Off state fault detected */
    {
      /* Clear all gate driver's Faults including latched faults */
        write_word_16b(0x10, 0x03);                              /* Write to gate driver FAULTS_CLR register */
        ...
        if (data == 0x00)
        {
        for (uint32_t y=10; y>0; y--)
          {
            for (uint32_t x = 1000000; x >0; x--) {};       /* create a delay loop of 2sec before exit OFF State */
          }
        }
    }
    else                                                          /* Off state fault detected and EN_DRV remain low */
        {
        MotorVar.error_status |= 1 << PMSM_FOC_EID_OFF_STATE_FAULT_POS;
        }
  }
}
...
void EDL7151_Update(void)
{
  ...
  if (GUIwrReg_6EDL7151_addr <= CFG_ADDR_MAX) /* Handle write request from GUI */
  {
    addr = GUIwrReg_6EDL7151_addr;
    write_word_16b(addr, GUIwrReg_6EDL7151_data); /* Write to 6EDL7151 */
    data = read_word_16b(addr); /* Read back from 6EDL7151 */
    Edl7151Reg.table[addr] = data; /* Update register table */
    GUIwrReg_6EDL7151_addr = 0x55; /* Indicate write operation is done */
  }
  if (WriteReg_6EDL7151_addr <= CFG_ADDR_MAX) /* Handle write request from control */
  {
    addr = WriteReg_6EDL7151_addr;
    write_word_16b(addr, WriteReg_6EDL7151_data); /* Write to 6EDL7151 */
    data = read_word_16b(addr); /* Read back from 6EDL7151 */
    Edl7151Reg.table[addr] = data; /* Update register table */
    WriteReg_6EDL7151_addr = 0x55; /* Indicate write operation is done */
  }

  /* Every 1ms, always read fault status register */
  Edl7151Reg.table[ADDR_FAULT_ST] = read_word_16b(ADDR_FAULT_ST);

  if (StatusRegister_count <= 6)
  {
    /* Every 1ms, read one of remaining 7 status register from address 1-7 */
    StatusRegister_count++;
    Edl7151Reg.table[StatusRegister_count] = read_word_16b(StatusRegister_count);
  }
  else
  {
    ...
    Edl7151Reg.table[addr] = read_word_16b(addr); /* Read one of configure registers */
    ...
    GuiMonitor_6EDL7151_value = Edl7151Reg.table[GuiMonitor_6EDL7151_addr];
  }
...
#endif /* (GUI_6EDL7151_INTEGRATION != DISABLED) */
